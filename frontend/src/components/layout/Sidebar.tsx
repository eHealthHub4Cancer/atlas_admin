    ...
    <Link
      ...
      className={cn(
        ...
        isCurrent
          ...
          : 'text-slate-900 dark:text-slate-100 hover:bg-slate-100 dark:hover:bg-dark-elevated hover:text-slate-950 dark:hover:text-white'
      )}
    >
      <item.icon
        className={cn(
          'h-5 w-5 shrink-0 transition-colors',
          isCurrent ? 'text-white' : 'text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white'
        )}
        aria-hidden="true"
      />
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <div className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:h-screen lg:w-64 lg:flex-col">
        <div className="flex h-full grow flex-col gap-y-5 overflow-y-auto border-r border-light-border dark:border-dark-border bg-white dark:bg-dark-card">
          <SidebarContent
            navigation={navigation}
            logo={logo}
            title={title}
